...
public class VentaDaoImpl implements VentaDao{
	...
	@Override
	public List<Venta> obtenerVentasPositivas() {
		List<Venta> dev = null;
		
		try{
			iniciaSession();
			dev = session.createQuery("from Venta where importe > 0 and total > 0").list();
		}finally {
			session.close();
		}
		return dev;
	...
	@Override
	public List<Venta> crearListado(Date fechaInicio, Date fechaFin) {
		List<Venta> dev = null;
		try{
			iniciaSession();
			String hql = "from Venta where fecha between :fechaInicio and :fechaFin";
			Query query = session.createQuery(hql);
