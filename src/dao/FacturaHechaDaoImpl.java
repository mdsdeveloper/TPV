...
public class FacturaHechaDaoImpl implements FacturaHechaDao{
	...
	public List<FacturaHecha> obtenerFacturas() {
		...
		try{
			iniciaSession();
			dev = session.createQuery("from FacturaHecha where total > 0").list();
		}finally {
			session.close();
		}
		return dev;
	...
	@Override
	public boolean hayFacturaHechaConIdVenta(Long idVenta) {
		
		FacturaHecha dev = null;
		
		try{
			iniciaSession();
			String hql = "from FacturaHecha where idVenta = :idVenta";
			Query query = session.createQuery(hql);
			query.setParameter("idVenta", idVenta);
//			dev = query != null ? query.getSingleResult() : null;
