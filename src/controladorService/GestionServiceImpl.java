...
import dao.ListaTicketDao;
import dao.ListaTicketDaoImpl;
...
public class GestionServiceImpl implements GestionService{
	...
	private ListaTicketDao listaTicketDao = new ListaTicketDaoImpl();
	...
	public String[][] rellenaTablaEmpleados() {
		...
		if(lista != null){
			...
			while(it.hasNext()){
				...
				dev[i][6] = empleado.isActivo() ? "Activo" : "Desactivo";
				i++;		
			}
		}
	...
	public void ActivarEmpleado(Empleado empleado) {
		Empleado empleadoActual = null;
		if(empleado != null){
			empleadoActual = empleadoDao.readEmpleado(empleado.getIdEmpleado());
		}
		if(empleadoActual != null){
			...
			empleadoActual.setActivo(true);
			empleadoActual.setPassword(!empleado.getPassword().equals("") ? empleado.getPassword() : empleadoActual.getPassword());
	...
	public void modificarTicket(Ticket ticket) {
		...
		if(ticket != null){
			ticketActual = ticketDao.readTicket(ticket.getIdTicket());
		}
		if(ticketActual != null){
			ticketActual.setCifNif(ticket.getCifNif());
			ticketActual.setDevuelto(ticket.getDevuelto());
			ticketActual.setDireccionEmpresa(ticket.getDireccionEmpresa());
			ticketActual.setEmpleado(ticket.getEmpleado());
			ticketActual.setFecha(ticket.getFecha());
			ticketActual.setFormaPago(ticket.getFormaPago());
			ticketActual.setHora(ticket.getHora());
			ticketActual.setNombreEmpresa(ticket.getNombreEmpresa());
			ticketActual.setNumeroDigito(ticket.getNumeroDigito());
			ticketActual.setTelefonoEmpresa(ticket.getTelefonoEmpresa());
			ticketActual.setTotal(ticket.getTotal());
			ticketDao.upDateTicket(ticketActual);
		}
	...
	public String[][] rellenarTablaVentas() {
		...
		Long idEmpleado;
		Empleado empleado;
		lista = ventaDao.obtenerVentasPositivas() != null ? ventaDao.obtenerVentasPositivas() : null;
		...
		if(lista != null){
			...
			while(it.hasNext()){
				venta = it.next();
				//if(UtilService.getEmpleadoLogado().getIdEmpleado() == venta.getEmpleado()){
					dev[i][0] = venta.getIdVenta().toString();
					dev[i][1] = venta.getFecha().toString();
					idEmpleado = venta.getEmpleado();
					empleado = empleadoDao.readEmpleado(idEmpleado) != null ? empleadoDao.readEmpleado(idEmpleado) : UtilService.getEmpleadoLogado();
					...
					if(venta.getIdLineaVentaRealizada() == null){
						dev[i][6] = "9999";	
					}else{
	...
	public String[][] rellenarTablaFacturas() {
		...
		String [][] dev = new String[lista.size()][18];
		...
		if(lista != null){
			...
			while(it.hasNext()){
				...
				dev[i][17] = facturaHecha.getIdVenta().toString();
				i++;						
			}
		}
		return dev;
	...
	public String[][] rellenarVentas() {
		...
		if(lista != null){
			...
			while(it.hasNext()){
				venta = it.next();						
				dev[i][0] = venta.getIdVenta().toString();
				dev[i][1] = empleadoDao.readEmpleado(venta.getEmpleado()) != null ? empleadoDao.readEmpleado(venta.getEmpleado()).getNombre() : UtilService.getEmpleadoLogado().getNombre();
	...
	public void eliminarFacturaHecha(String idFactura) {
		...
		if(facturaHecha != null){
			...
			List<FacturaHecha> listaFacturas = facturaHechaDao.obtenerFacturas();
			for(int i=0; i<listaFacturas.size(); i++){
				listaFacturas.get(i).setNumero(i + 1);
				facturaHechaDao.upDateFacturaHecha(listaFacturas.get(i));
			}
		}
		
	}
	@Override
	public String[][] rellenarTablaTicket(String nombre) {
		List<Ticket> lista = null;
		int i=0;
		if(nombre.equals("PantallaVentas.OpcionesNuevo")){
			lista = ticketDao.obtenerTickets(new Date());
		}else{
			lista = ticketDao.obtenerTickets(null);
		}
		...
		if(lista != null){
			...
			while(it.hasNext()){
				ticket = it.next();						
				dev[i][0] = ticket.getIdTicket().toString();
				dev[i][1] = ticket.getFecha();
				dev[i][2] = ticket.getHora();
				dev[i][3] = ticket.getEmpleado();
				dev[i][4] = ticket.getFormaPago() == false ? "Efectivo" : "Tarjeta";
				dev[i][5] = ticket.getTotal().toString();
				dev[i][6] = ticket.getDevuelto().toString();
				i++;
			}
		}
	...
	public void eliminarTicket(String idTicket) {
		Long id = Long.parseLong(idTicket);
		Ticket ticket;
		List<ListaTicket> listaTicket;
		listaTicket =  listaTicketDao.obtenerListaTicketsById(Long.parseLong(idTicket));
		ticket = ticketDao.readTicket(id);
		if(ticket != null && listaTicket != null){
			ticketDao.deleteTicket(ticket);			
			for (ListaTicket listaTicket2 : listaTicket) {
				listaTicketDao.deleteListaTicket(listaTicket2);
			}
		}
		
	}
	@Override
	public List<Venta> crearListado(Date fechaInicio, Date fechaFin, List<String> listaCheck) {
		List<Venta> lista = ventaDao.crearListado(fechaInicio, fechaFin);
	...
	@Override
	public Venta obtenerIdVentaRealizadaByIdVenta(Long idVenta) {
		Venta dev = null;
		dev = ventaDao.readVenta(idVenta);
		return dev;
	}
