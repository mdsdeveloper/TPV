...
public class AppServiceImpl implements AppService{
	...
	private DevolucionDao devolucionDao = new DevolucionDaoImpl();
	...
	public Long crearTicket(String fecha, String hora, String empleado, String nombreEmpresa, String direccionEmpresa,
		...
		ticket.setCifNif(cifNif);
		ticket.setDireccionEmpresa(direccionEmpresa);
		ticket.setEmpleado(empleado);
		ticket.setFecha(fecha);
		ticket.setFormaPago(formaPago);
		ticket.setHora(hora);
		ticket.setNombreEmpresa(nombreEmpresa);
		ticket.setNumeroDigito(numeroDigito);
		ticket.setTelefonoEmpresa(telefonoEmpresa);
		ticket.setTotal(aux);
		ticket.setDevuelto(devuelto);
		
		dev = ticketDao.createTicket(ticket);
		return dev;
	}

	@Override
	public void crearListaTicket(Long idTicket, String nombreProducto, String cantidad, String precioProducto, String total) {
		ListaTicket listaTicket = new ListaTicket();
		
		listaTicket.setCantidadProducto(cantidad);
		listaTicket.setIdTicket(idTicket);
		listaTicket.setNombreProducto(nombreProducto);
		listaTicket.setPrecioProducto(precioProducto);
		listaTicket.setTotal(total);
		
		
		listaTicketDao.createListaTicket(listaTicket);
	...
	public Ticket obtenerTicketById(String idTicket) {
		Ticket dev = null;
		dev = ticketDao.readTicket(Long.valueOf(idTicket));
		return dev;		
	}

	@Override
	public List<ListaTicket> obtenerListaTicketById(Long idTicket) {
		List<ListaTicket> dev;
		
		dev = listaTicketDao.obtenerListaTicketsById(idTicket);
		return dev;
	}

	@Override
	public void crearDevolucion(modelo.Devolucion dev) {
		devolucionDao.createDevolucion(dev);
	...
	public boolean obtenerfacturasHechaByIdVenta(Long idVenta) {
		boolean dev;
		dev = facturaHechaDao.hayFacturaHechaConIdVenta(idVenta);
		return dev;
	}
