 ...
 *
 */
public class FormularioFacturas extends JDialog implements ActionListener{
	...
	public static void main(String[] args) {
		try {
			FormularioFacturas dialog = new FormularioFacturas();
...
private void construirTabla() {
		...
		String cabecera[] = {"IdFactura","Cliente","CIF/NIF","Teléfono","Email","Dirección","Fecha", "Empleado","F.pago","N.factura","Precio","Nombre","R.social","CIF/NIF","Dirección","Teléfono","Total","idVenta"};	
		...
		table.getTableHeader().getColumnModel().getColumn(17).setMaxWidth(0);
		table.getTableHeader().getColumnModel().getColumn(17).setMinWidth(0);
	...
	private void fillForm() {
		...
		idVenta = Long.valueOf((String)modeloTabla.getValueAt(filasSeleccionadas, 17));
	}
	...
	private FacturaHecha fillObject(){
		...
		if(validar()){	
			...
			Venta venta = service.obtenerIdVentaRealizadaByIdVenta(idVenta);
			facturaHecha.setIdVentaRealizada(venta.getIdLineaVentaRealizada());
			facturaHecha.setNumero(Integer.valueOf(txtNumero.getText()));
			BigDecimal aux = new BigDecimal(txtTotal.getText());
			Float base = aux.floatValue();
			Float dividendo = totalIva.floatValue();
			base = base/dividendo;
			BigDecimal baseImponible = new BigDecimal(base);
			facturaHecha.setBaseImponible(baseImponible.setScale(2, RoundingMode.HALF_DOWN));
			facturaHecha.setTotal(aux);	
