			...
			if(empleado != null && !usuario.equals(ConstantesUtil.superAdmin)){
				if(nombre.equals(ConstantesUtil.administrador)){					
					if(empleado.getCargo().equals(ConstantesUtil.administrador)){
						if((empleado.getNombre().equals(usuario) && empleado.getPassword().equals(password)) && empleado.isActivo()){
							dev = true;
							UtilService.empleado = empleado;
							FormularioPrincipalNuevo formularioGestion = new FormularioPrincipalNuevo(anchoVentana, altoVentana,ancho, alto);
