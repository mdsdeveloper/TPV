...
public class FormularioEmpleados extends JDialog implements ActionListener{
	...
	private String nombre, cargo, dni, email, telefono, idEmpleado, password, activo;
	private char[] pass;
...
//		panelForm.add(tfCargo);
	...
	private void estadoInicial() {
		...
		btnEliminar.setText(ConstantesUtil.eliminar);
	...
	private void construirTabla() {
		...
		String cabecera[] = {"IdEmpleado","Nombre","Cargo","DNI","Email","Teléfono", "Activo"};	
	...
	private void fillForm() {
		activo = (String) modeloTabla.getValueAt(filasSeleccionadas, 6);
		if(activo == "Desactivo"){
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(source.getText().equals(ConstantesUtil.salir)){
			FormularioPrincipalNuevo.btnGestionEmpleado.setEnabled(true);
			dispose();
			...
			clearForm();
			estadoInicial();
		}else if(source.getText().equals("ACTIVAR")){
			if(idEmpleado != null && !idEmpleado.equals("")){
				Empleado empleado = fillObject();	
				if(empleado != null){
					empleado.setIdEmpleado(Long.parseLong(idEmpleado));
					empleado.setActivo(true);
					service.ActivarEmpleado(empleado);
					clearForm();
					estadoInicial();
				}				
			}
